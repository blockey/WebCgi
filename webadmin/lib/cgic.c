...
int main(int argc, char *argv[]) {
	...
	char info[1024] = {0};
    sprintf(info,"cgiContentLength: %d!\n",cgiContentLength);
    webLog(info);
...
static cgiParseResultType cgiParsePostFormInput() {
	...
	input = (char *) malloc(sizeof(char)*(cgiContentLength+1));
	if (!input) {
		return cgiParseMemory;	
	}
    
    memset(input, 0, cgiContentLength+1); 
    int nRead = fread(input, 1, cgiContentLength, cgiIn);
	if (nRead != cgiContentLength) 
	{
		return cgiParseIO;
	}
    
    sprintf(info,"CgiInput[Len=%d]: %s!\n", strlen(input),input);
    webLog(info);
